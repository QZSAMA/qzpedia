...
| 组件 | 选型 | 版本 | 理由 | 费用 |
|------|------|------|------|------|
| 基础项目 | WeClone | latest | 一站式方案，16k+ stars 验证 | 免费 |
| 基础模型 | Qwen2.5-7B-Instruct | 7B | 默认配置，效果够用 | 免费 |
| 微调框架 | LLaMA Factory | latest | WeClone 底层依赖 | 免费 |
| 微调方法 | QLoRA 4-bit | - | 6GB 显存可用 | 免费 |
| 数据导出 | PyWxDump | latest | 微信聊天记录导出 | 免费 |
| 推理服务 | vLLM / transformers | - | 高效推理 | 免费 |
| Web UI | Gradio | 4.x | 快速搭建界面 | 免费 |
| API 服务 | FastAPI | 0.100+ | 轻量 API | 免费 |
| 语音克隆 | Spark-TTS (可选) | - | 增强真实感 | 免费 |
| 包管理 | uv | latest | 快速安装 | 免费 |
...
> 无 GPU 时可使用免费云端进行微调：Google Colab (免费 GPU)、Kaggle (免费 T4)，本地仅推理。
...
6. **免费云端限制**：Colab/Kaggle 免费版有运行时长和 GPU 类型限制
