...
| 记忆存储 | 向量数据库 | sqlite-vec (MVP) → Qdrant/Postgres+pgvector (扩展) | 零依赖起步，渐进升级 |
| 记忆存储 | 时序知识图谱 | 自研时序图谱引擎 (借鉴 Zep/Graphiti 设计) | 时序推理是核心需求，自研更可控 |
| 记忆存储 | 认知分离层 | 四网络逻辑分离 (借鉴 Hindsight) | 事实-观点分离是分身关键能力 |
| 记忆存储 | 原始文件 | Markdown + Git (借鉴 GBrain) | 数据主权，可版本控制 |
| 记忆索引 | 嵌入模型 | bge-m3 (本地) 或 OpenAI text-embedding-3 | 中英文效果好 |
| RAG 引擎 | 检索框架 | LlamaIndex | 个人 RAG 生态最成熟 |
| LLM 推理 | 基础模型 | Qwen2.5-14B-Instruct (本地) 或 API | 中文能力强，14B 效果好 |
| 人格微调 | 微调框架 | LLaMA Factory + LoRA/CharLoRA | WeClone 验证过的流程 |
| 语音克隆 | TTS | Spark-TTS 或 GPT-SoVITS | 中文语音克隆效果好 |
...
1. **向量索引 (Qdrant)**：语义相似度检索，"找到和这件事相关的内容"
2. **时序图谱 (Zep)**：时间推理检索，"2019年夏天我去了哪里"
...
def retrieve_memories(query: str, top_k: int = 10) -> list[Memory]:
    # 并行检索三个索引
    vector_results = qdrant.search(query_embedding(query), limit=top_k * 2)
    temporal_results = zep.search(query, time_filter=extract_time(query))
...
│   │   ├── sqlite_store.py   # SQLite 原始存储
│   │   ├── qdrant_store.py   # Qdrant 向量存储
│   │   └── zep_store.py      # Zep 时序图谱存储
│   │
...
| Qdrant 向量库 | 高 (生产级) | 低 | 本地部署简单 |
| Zep 时序图谱 | 中 (活跃开发) | 中 | 可降级为 SQLite 时序查询 |
...
| 时序图谱复杂度 | 中 | Phase 1 可先用简单时序索引，Phase 2 再引入 Zep |
| 长期维护成本 | 中 | 模块化设计，组件可替换 |

## 10. 实施路线图

### Phase 1: 记忆引擎 (MVP)
- Week 1-2: 项目骨架 + 数据模型 + SQLite 存储
- Week 3-4: Markdown 日记采集器 + CLI 工具
- Week 5-6: Qdrant 向量索引 + 语义检索
- Week 7: Screenpipe 集成
- Week 8: 微信聊天记录导入

### Phase 2: 记忆检索
- Week 9-10: RAG 管道 + 查询理解
- Week 11-12: Zep 时序图谱集成
- Week 13: 检索融合 + 重排序
