...
本报告对当前 AI Agent 记忆领域的 **10 个主流方案** 进行了系统性深度分析，涵盖开源框架（GBrain、Hindsight、Mem0、Zep、Cognee、TencentDB Agent Memory、agentmemory）、学术架构（MemoryOS、PersonalAI）和云服务方案（腾讯云向量数据库）。基于 Chronipedia 项目的核心需求——"时序记忆 + 人格模拟 + 本地优先 + 个人免费"，本报告提出**混合分层记忆架构**推荐方案：以 GBrain 的 Markdown-first 模式为数据底座，以 Hindsight 的四网络认知分离为记忆组织范式，借鉴 Zep/Graphiti 的时序知识图谱设计思想自研轻量时序引擎，构建 Chronipedia 专属的记忆模块。所有推荐方案均可在零费用、纯本地的条件下运行。
...
| 方案 | 类型 | 核心架构 | 时序推理 | 本地优先 | 人格建模 | 个人免费 | Stars |
|------|------|----------|----------|----------|----------|----------|-------|
| **GBrain** | 开源 | Markdown + Postgres + pgvector | 无 | 是 | 无 | ✅ 完全免费 | 23.8K |
| **Hindsight** | 开源 | 四网络 + TEMPR + CARA | 原生支持 | 是 | 原生支持 | ✅ 完全免费 | 12.8K |
| **Mem0** | 开源+商业 | 向量DB + 可选知识图谱 | 无 | 部分 | 无 | ⚠️ 图谱付费墙 | 48K |
| **Zep/Graphiti** | 开源+商业 | 时序知识图谱 | 核心能力 | 部分 | 无 | ⚠️ 自托管复杂 | 24K |
| **Cognee** | 开源 | 知识图谱 + 向量 + 关系DB | 部分 | 是 | 无 | ✅ 完全免费 | 12K |
| **TencentDB AM** | 开源 | L0-L3 蒸馏 + SQLite | 部分 | 是 | L3层 | ✅ 完全免费 | 新项目 |
| **agentmemory** | 开源 | 三流融合 + SQLite | 无 | 是 | 无 | ✅ 完全免费 | 23K |
| **MemoryOS** | 学术 | 三层分级存储 | 部分 | 是 | LPM层 | ✅ 需自行实现 | 学术 |
| **PersonalAI** | 学术 | 混合知识图谱 | 部分 | 是 | 无 | ✅ 需自行实现 | 学术 |
| **腾讯云向量DB** | 商业 | 云端向量数据库 | 无 | 否 | 无 | ❌ 必须付费 | 商业 |
...
**对 Chronipedia 的启示**：四网络认知分离是数字分身的关键能力——分身需要区分"我做过什么"和"我相信什么"。CARA 的偏好一致性机制直接服务于"像你一样思考"的目标。**个人免费使用策略**：Hindsight 大量依赖 LLM 调用，但可通过 Ollama + Qwen 本地模型替代云端 API，实现零成本运行。
...
**对 Chronipedia 的启示**：Mem0 的简洁 API 设计值得借鉴，但架构上无法满足时序推理和人格建模的核心需求。**个人免费使用不推荐**：知识图谱功能被 $249/月付费墙限制，自托管体验差，且核心能力（时序推理、人格建模）缺失。
...
**对 Chronipedia 的启示**：时序推理是 Chronipedia 最核心的需求，Zep/Graphiti 在这个维度上是最佳选择。但需要解决自托管复杂度问题。**个人免费使用策略**：不直接使用 Zep，而是借鉴其时序图谱设计思想，在 Layer 2 用更轻量的 SQLite + sqlite-vec 自研实现，避免 Neo4j/FalkorDB 的运维负担。
...
| **Hindsight** | Postgres + pgvector | 是 | 中等 | ✅ (需本地LLM) |
| **Zep/Graphiti** | Neo4j/FalkorDB | 复杂 | 较长 | ⚠️ 自托管可用但重 |
| **Mem0** | 向量DB | 部分 | 简单 | ⚠️ 核心功能付费 |
...
|------|----------|------|--------|
| **GBrain** | ✅ | 无 | ⭐⭐⭐⭐⭐ 数据底座首选 |
| **Hindsight** | ✅ | 需本地 LLM (Ollama) | ⭐⭐⭐⭐ 认知层借鉴 |
| **TencentDB AM** | ✅ | 功能有限 | ⭐⭐⭐⭐ 轻量起步首选 |
| **agentmemory** | ✅ | 仅 Coding Agent | ⭐⭐⭐ 检索架构借鉴 |
| **Cognee** | ✅ | 社区较小 | ⭐⭐⭐ ECL 管道借鉴 |
| **MemoryOS** | ✅ | 需自行实现 | ⭐⭐⭐ 架构参考 |
| **Zep/Graphiti** | ⚠️ | 自托管需 Neo4j | ⭐⭐ 借鉴设计，不自托管 |
| **Mem0** | ⚠️ | 图谱付费墙 | ⭐ 不推荐 |
| **腾讯云向量DB** | ❌ | 必须付费 | ✗ 排除 |
...
- Hindsight 的四网络认知分离最适合数字分身，但运行成本高（可用本地 LLM 缓解）
- Zep 的时序知识图谱最强，但自托管复杂（借鉴设计，不自托管）
- TencentDB AM 的零依赖蒸馏架构最轻量，但功能有限

**个人免费使用约束下**，还需排除：Mem0（图谱付费墙）、腾讯云向量DB（必须付费），并将 Zep 从"直接使用"降级为"借鉴设计"。

**结论：采用混合分层架构，取各家之长，全程零费用。**
...
│                              ▲                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  Layer 2: 时序层 (Temporal Layer) — 借鉴 Zep/Graphiti 设计，自研轻量实现  │    │
│  │                                                              │    │
│  │  时序知识图谱：每条边携带 (valid_from, valid_to, confidence) │    │
│  │  支持：时间范围查询、事实有效期、矛盾检测、多跳推理          │    │
│  │  存储：SQLite + sqlite-vec (全程零外部依赖)                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
...
| Layer 2 时序层 | Zep/Graphiti 设计思想 (自研实现) | SQLite + sqlite-vec | 时序推理、事实有效期 | ✅ |
...
因此借鉴其时序图谱设计思想，在 Layer 2 用 SQLite + sqlite-vec 自研轻量时序引擎，避免外部图数据库依赖。自研引擎在 10 万条记忆以内完全够用，且零运维成本。

#### 4.4.4 存储策略：零依赖优先，渐进式升级

```
Phase 1 (MVP):   SQLite + sqlite-vec + FTS5          ← 零外部依赖，零费用
                 ↓ 数据量增长
Phase 2 (扩展):  Postgres + pgvector + tsvector      ← 仍免费，需自托管
                 ↓ 需要更强时序推理
Phase 3 (增强):  Postgres + pgvectorscale(DiskANN)   ← 仍免费，性能更强
```

**个人免费使用核心原则**：所有阶段均使用开源免费组件，不引入任何付费服务。SQLite 在 10 万条记忆以内完全够用，且零运维。Postgres 升级路径也是完全免费的开源方案，仅在数据量增长后按需切换。
...
- **Hindsight** 贡献了四网络认知分离范式和偏好一致性机制（通过 Ollama 本地模型实现零成本运行）
- **Zep/Graphiti** 贡献了时序知识图谱的设计思想（借鉴设计，自研轻量实现，避免 Neo4j 运维负担）
- **Cognee** 贡献了 ECL 管道和自动知识构建流程
- **TencentDB AM** 贡献了四层蒸馏架构和零依赖部署模式
- **agentmemory** 贡献了三流融合检索的工程实现
- **MemoryOS** 贡献了三层分级存储和热度驱动的层间迁移机制

**个人免费使用排除**：Mem0（图谱付费墙）、腾讯云向量DB（必须付费）不纳入架构。

这个混合架构确保了 Chronipedia 在每个核心能力维度上都有最佳实践支撑，同时保持本地优先、渐进式演化、**全程零费用**的项目理念。
